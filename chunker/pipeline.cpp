...
void outerDecoder(std::vector<uint8_t> encodedVec, schifra::reed_solomon::erasure_locations_t erasures) {
    const std::size_t field_descriptor = 8;
    const std::size_t generator_polynomial_index = 0;
    const std::size_t generator_polynomial_root_count = 50;
    ...
    for (int i = 0; i < encodedVec.size() - 1; i += 255) {
        std::vector<uint8_t> subvector = {encodedVec.begin() + i, encodedVec.begin() + 255 + i};

        std::string message = "";
        for (int k = 0; k < 205; k++) {
            message += char(subvector[k]);
        }
        ...
        for (int j = 0; j < subvector.size(); j++) {
            block.data[j] = subvector[j];
        }

        if (!decoder.decode(block, erasures)) {
            std::cout << "Error - Critical decoding failure! "
                      << "Msg: " << block.error_as_string() << std::endl;
        } else if (!schifra::is_block_equivelent(block, message)) {
            std::cout << "Error - Error correction failed!" << std::endl;
